...
public sealed class CustomerSimulationServiceTests
{
    ...
    public async Task GetTrackingAsync_InterpolatesPositionDuringTrip()
    {
        ...
        Assert.Equal("Seu pacote está a caminho", tracking.FriendlyStatus);
        Assert.True(tracking.CurrentPosition.X > 0);
        Assert.True(tracking.RemainingDistance > 0);
    }

    [Fact]
    public async Task GetTrackingAsync_MarksCustomerAsReceivedWhenDroneReachesDestinationBeforeReturn()
    {
        await using var dbContext = TestDbContextFactory.Create();
        var drone = new Drone { Code = "DRN", Name = "Drone", MaxPackageWeightKg = 10, MaxRangeKm = 100, BatteryLevelPercent = 100, AverageSpeedKmPerHour = 60, BatteryConsumptionPercentagePerKm = 1.5m, IsActive = true };
        var order = CreateOrder();
        ...
        dbContext.Deliveries.Add(new Delivery
        {
            ...
            EndY = 0,
            EstimatedDistanceKm = 20,
            EstimatedBatteryConsumptionPercent = 10,
            EstimatedDurationMinutes = 1,
            AllocatedAt = new DateTime(2026, 7, 25, 11, 59, 0, DateTimeKind.Utc),
            CreatedAtUtc = new DateTime(2026, 7, 25, 11, 59, 0, DateTimeKind.Utc),
            LoadingStartedAtUtc = new DateTime(2026, 7, 25, 11, 59, 0, DateTimeKind.Utc),
            FlyingStartedAtUtc = new DateTime(2026, 7, 25, 11, 59, 30, DateTimeKind.Utc),
            DeliveringStartedAtUtc = new DateTime(2026, 7, 25, 11, 59, 50, DateTimeKind.Utc),
            ReturningStartedAtUtc = new DateTime(2026, 7, 25, 12, 0, 0, DateTimeKind.Utc),
            CompletedAtUtc = new DateTime(2026, 7, 25, 12, 2, 0, DateTimeKind.Utc)
        });
        await dbContext.SaveChangesAsync();
        var service = CreateService(dbContext);
        ...
        Assert.Equal("Entrega concluída", tracking.FriendlyStatus);
        Assert.Equal("Received", tracking.InternalStatus);
        Assert.Equal(100, tracking.ProgressPercentage);
        Assert.Equal(0, tracking.RemainingDistance);
        Assert.Equal(order.DestinationX, tracking.CurrentPosition.X);
        Assert.Equal(order.DestinationY, tracking.CurrentPosition.Y);
    }
